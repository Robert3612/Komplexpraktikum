...

{
    ...
    ("type,t", po::value<char>()->default_value('i'), "write every output into file");
    ...
    if (vm.count("seed"))
    {
        my_Datagenerator help(vm["seed"].as<int>(), vm["distr"].as<char>(), t);
        generator = help;
    }
    else
    {
        my_Datagenerator help(vm["distr"].as<char>(), t);
        generator = help;
    }

    // erstelle Dataset mit dem Datentyp der in der command line angegeben wurde
    switch (t)
    {
    case 'i':
    {
        Dataset<int> dataset(c, r);
    ...
    case 's':
    {
        Dataset<std::string> dataset(c, r);
        dataset.fill(&generator);
        // gibt alle Werte des Datasets aus
        dataset.printData();
        break;
